...
it('If businessServiceDuration = 30m & If customer defines the ServiceDuration = 30', function(done) {
...
it('If AllowFutureBookingFor = 60d & If customer defines the AllowFutureBookingFor = 60', function(done) {
  ...
  it('If customer try to find available slots of past duration', function(done) {
    ...
    const BusinessServiceId = getTestData("BusinessServiceId");
      agent
          .get(`/api/v1/appointments/business/${BusinessId}/node/${BusinessNodeId}/service/${BusinessServiceId}/slots?toDate=2023-07-30`)
          .set('Content-Type', 'application/json')
          .set('x-api-key', 'T26BP24-MRGMRYE-JB352V-NC93PY0')
          .expect(response => {
            expect(response.body).to.have.property('Status');
  ...
  export const loadAppointmentCreateModel = async (
    ...
    export const loadAppointmentGetModel = async (
      ...
      function loadNotificationQueryString() {
        ...
        const queryString = '?fromDate=2023-07-07&toDate=2023-07-011'
        return queryString;
    }