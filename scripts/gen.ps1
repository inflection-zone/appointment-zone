foreach ( $project in $args ) {
    ...
    ./node_modules/.bin/grpc_tools_node_protoc -I $project/user/proto                                       `
    --js_out=import_style=commonjs:$project/user/proto                                  `
    --grpc_out=grpc_js:$project/user/proto                                              `
    $project/user/proto/user.proto;
    # Generate only Protobuf code for all the other .proto files (if any)
    # (eg: calculator/sum.proto)
    Get-ChildItem -Filter *.proto -Recurse $project/user/proto -Exclude $project/user/.proto | ForEach-Object { `
        $file = "./$project/user/proto/" + $_.Name;                                  `
        ./node_modules/.bin/grpc_tools_node_protoc -I $project/user/proto                                    `
        --js_out=import_style=commonjs:$project/user/proto                               `
        $file `
    }
